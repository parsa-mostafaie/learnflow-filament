...
@props(['filterKey'])
@if ($isTailwind)
  <button wire:click="resetFilter('{{ $filterKey }}')" type="button" @class([
      'flex-shrink-0 ms-0.5 h-4 w-4 rounded-full inline-flex items-center justify-center focus:outline-none',
...
@else
  <a href="#" wire:click="resetFilter('{{ $filterKey }}')" @class([
      'text-white ms-2' => $isBootstrap,
  ])>
    <span @class([
