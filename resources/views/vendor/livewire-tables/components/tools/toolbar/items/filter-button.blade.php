      ...
      <button type="button" @class([
        ...
        @if ($count = $this->getFilterBadgeCount())
          <span @class([
              ...
              'ms-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium leading-4 bg-indigo-100 text-indigo-800 capitalize dark:bg-indigo-200 dark:text-indigo-900' =>
                  $this->isTailwind,
          ])>
            {{ $count }}
          </span>
        ...
        @if ($this->isTailwind)
          <x-heroicon-o-funnel class="-me-1 ms-2 h-5 w-5" />
        @else
          <span @class([
