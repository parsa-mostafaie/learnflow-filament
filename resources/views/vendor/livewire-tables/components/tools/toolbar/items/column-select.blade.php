...
@if ($isTailwind)
  <div
    class="@if ($this->getColumnSelectIsHiddenOnMobile()) hidden sm:block @elseif ($this->getColumnSelectIsHiddenOnTablet()) hidden md:block @endif mb-4 w-full md:w-auto md:mb-0 md:ms-2">
    <div x-data="{ open: false, childElementOpen: false }" @keydown.window.escape="if (!childElementOpen) { open = false }"
      x-on:click.away="if (!childElementOpen) { open = false }" class="inline-block relative w-full text-start md:w-auto"
      ...
      <div>
        <span class="rounded-md shadow-sm">
          <button x-on:click="open = !open" type="button"
            ...
            <x-heroicon-m-chevron-down class="-me-1 ms-2 h-5 w-5" />
          </button>
        </span>
      </div>
          ...
          <div class="p-2" role="menu" aria-orientation="vertical" aria-labelledby="column-select-menu">
            <div wire:key="{{ $tableName }}-columnSelect-selectAll-{{ rand(0, 1000) }}">
              <label wire:loading.attr="disabled"
                ...
                <span class="ms-2">{{ __($this->getLocalisationPath . 'All Columns') }}</span>
              </label>
            </div>
              ...
              <div wire:key="{{ $tableName }}-columnSelect-{{ $loop->index }}">
                <label wire:loading.attr="disabled" wire:target="selectedColumns"
                  ...
                  <input
                    ...
                    wire:model.live="selectedColumns" wire:target="selectedColumns" wire:loading.attr="disabled"
                    type="checkbox" value="{{ $columnSlug }}" />
                  <span class="ms-2">{{ $columnTitle }}</span>
                </label>
              </div>
            @endforeach
          </div>
          ...
          <div wire:key="{{ $tableName }}-columnSelect-{{ $loop->index }}" @class([
            ...
            @if ($isBootstrap4)
              <label wire:loading.attr="disabled" wire:target="selectedColumns"
                ...
                <input wire:model.live="selectedColumns" wire:target="selectedColumns" wire:loading.attr="disabled"
                  type="checkbox" value="{{ $columnSlug }}" />
                <span class="ms-2">
                  {{ $columnTitle }}
                </span>
            ...
            @elseif($isBootstrap5)
              <input wire:model.live="selectedColumns" wire:target="selectedColumns" wire:loading.attr="disabled"
                type="checkbox" class="form-check-input" value="{{ $columnSlug }}" />
              <label wire:loading.attr="disabled" wire:target="selectedColumns"
