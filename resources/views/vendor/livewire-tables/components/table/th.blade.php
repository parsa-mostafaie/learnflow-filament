...
<th
  {{ $attributes->merge($customThAttributes)->class([
          ...
          'px-6 py-3 text-start text-xs font-medium whitespace-nowrap uppercase tracking-wider' =>
              $this->isTailwind &&
      ...
      @if ($this->isTailwind)
        <button wire:click="sortBy('{{ $column->getColumnSortKey() }}')"
          {{ $attributes->merge($customSortButtonAttributes)->class([
                  ...
                  'flex items-center space-x-1 text-start text-xs leading-4 font-medium uppercase tracking-wider group focus:outline-none' =>
