...
@if ($this->bulkActionsAreEnabled() && $this->hasBulkActions())
  ...
  <x-livewire-tables::table.tr.plain x-cloak x-show="selectedItems.length > 0 && !currentlyReorderingStatus"
    wire:key="{{ $tableName }}-bulk-select-message" @class([
        ...
        <div wire:key="{{ $tableName }}-all-selected">
          ...
          <button x-on:click="clearSelected" wire:loading.attr="disabled" type="button"
            {{ $this->getBulkActionsRowButtonAttributesBag->class([
                'ms-1 underline text-sm leading-5 font-medium focus:outline-none focus:underline transition duration-150 ease-in-out' =>
                    $this->isTailwind && ($this->getBulkActionsRowButtonAttributes['default-styling'] ?? true),
                'text-blue-600 text-gray-700 focus:text-gray-800 dark:text-white dark:hover:text-gray-400' =>
                    $this->isTailwind && ($this->getBulkActionsRowButtonAttributes['default-colors'] ?? true),
                'btn btn-primary btn-sm' =>
                    $this->isBootstrap && ($this->getBulkActionsRowButtonAttributes['default-styling'] ?? true),
            ]) }}>
